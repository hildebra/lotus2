...
sub usage; sub help; sub frame; 
sub readPaths; sub readPaths_aligners;
sub uclustOTUs; sub buildOTUs;
sub mergeUCs; sub delineateUCs; sub cutUCstring; sub uniq;
sub cdhitOTUs; sub checkBlastAvaila;

sub makeAbundTable2; sub readMap;
sub assignTaxOnly;

sub writeUTAXhiera; sub doDBblasting;
sub numberOTUs; sub getTaxForOTUfromRefBlast; 
sub get16Sstrand; sub splitBlastTax;
sub extractTaxoForRefs;
sub calcHighTax; sub buildTree; sub combine;
sub contamination_rem; sub readOTUmat;
#fasta IO 
sub readFastaHd; sub replaceFastaHd;
sub splitFastas; sub extractFastas;
sub writeFasta; sub readFasta;
sub revComplFasta; sub reverse_complement_IUPAC;
sub newUCSizes;
sub readLinkRefFasta; sub readTaxIn;
sub biomFmt; sub printL;
sub finWarn; sub printWarnings; #1 to add warnings, the other to print these
sub forceMerge_fq2fna; sub clean_otu_mat;
sub systemL;
sub swarmClust; sub swarm4us_size;
sub dnaClust2UC;
sub chimera_rem; 
sub checkLtsVer;
sub ITSxOTUs; sub checkXtalk;
sub systemW;
...
my $usBin = "";my $swarmBin = ""; my $VSBin = ""; my $VSBinOri="";my $VSused = 1;
my $cdhitBin = "";my $dnaclustBin = "";
my $LCABin = "";
my $sdmBin = "./sdm";
my $mjar = "";#RDP multiclassifier java archieve (e.g. /YY/MultiClassifier.jar)
my $rdpjar = "";#alternatively leave this "" and set environmental variabel to RDP_JAR_PATH, as described in RDP documentary
...
my $ClusterPipe_pre = "1"; my $ClusterPipe = 1; #use UPARSE (1) or otupipe(0) or SWARM (2) or cd-hit(3), dnaclust (4), micca (5)
my $lotus_tempDir;
...
my $check_map = "";
my $lotusCfg = "lOTUs.cfg";
my $curSdmV = 1.50;
my $platform = "454"; #454, miSeq, hiSeq, PacBio
...
my $minBit = 120; my $minEval = 1e-14; #blast filtering; should be optimized to different platforms
...
my $doBlasting = -1; #$doBlasting 2:lambda, 1:blast, 0:RDP, -1: ini value, 3:utax
...
my $swarmClus_d = 1;
my $id_OTU_noise=.99;   # Id threshold for error correction (noise removal) round #Default: .99
my $uthreads = 1;
...
my $input;my $outdir;my $inq = ""; my $barcodefile = "";
my $map;my $exec=0; #my $highmem = 0;
my $sdmDerepDo=2;
if (!@ARGV) {
    usage();
    exit 1;
...
if ($greengAnno){
	$pseudoRefOTU=1;
	$refDBwanted="GG";$doBlasting_pre="2";
...
$dereplicate_minsize = $dereplicate_minsize_def if ($dereplicate_minsize == -1);
#die $refDBwanted."\n";
...
if ($input=~m/\*/){die "\"*\" not supported in input command. Please see documentation on how to set up the mapping file for several input files.";}
defined( $outdir ) or usage("-o option (output dir) is required\n");
if (!defined( $map) && $TaxOnly==0) { usage("-m missing option (mapping file)\n");}
if (!defined ($sdmOpt ) && $TaxOnly==0) {finWarn("WARNING:\n sdm options not set\n WARNING\n");}
...
if ($extendedLogs){
	system("mkdir -p $extendedLogD") unless (-d $extendedLogs);
}
#die "$sdmDerepDo \n";
...
$usearchVer = $1; my $usearchsubV = $2;
#if ($usearchVer == 9){printL "Usearch ver 9 currently not supported, please install ver 8.\n",39;}
if ($usearchVer > 11){ printL "Usearch ver $usearchVer is not supported.\n",55;
} elsif ($usearchVer >= 8 && $usearchVer < 9){printL "Usearch ver 8 is outdated, it is recommended to install ver 9.\nDownload from http://drive5.com/ and execute \n\"./autoInstall.pl -link_usearch [path to usearch9]\"\n",0;
} elsif ($usearchVer < 8){printL "Usearch ver 7 is outdated, it is recommended to install ver 9.\nDownload from http://drive5.com/ and execute \n\"./autoInstall.pl -link_usearch [path to usearch9]\"\n",0;}
...
if ($doBlasting == -1 ){
	if ($defDBset){
		...
		if (!-f $lambdaBin ){ 
			...
			elsif (!-f $blastBin && $refDBwanted ne "") { printL "Requested similarity search ($refDBwanted), but no suitable aligner (blast, lambda) binaries found!\n",50; }
		}
...
if ($ClusterPipe_pre eq "CD-HIT" || $ClusterPipe_pre eq "3"){
	$ClusterPipe = 3;
	...
	$ClusterPipe = 4;
	if (!-e $dnaclustBin){printL "No valid DNA clust binary found at $dnaclustBin\n",88;}
}
...
if ($REFflag){
	if ($ClusterPipe  == 0){printL ("otupipe does not support ref DB out clustering\nUse dnaclust instead\n",12);
	} elsif ($ClusterPipe  == 1){printL ("UPARSE does not support ref DB out clustering\nUse dnaclust instead\n",12);
	} elsif ($ClusterPipe  == 2){printL ("SWARM does not support ref DB out clustering\nUse dnaclust instead\n",12);
	}
	if ($refDBwanted eq ""){printL "You selected ref based OTU building, please set -refDB to \"SLV\", \"GG\", \"HITdb\", \"PR2\" or a custom fasta file.\n",22;}
...
if (substr($ampliconType,0,3) eq "ITS"){
	...
	if ($doBlasting  == 0 || (!-f $blastBin && !-f $lambdaBin) || @TAX_REFDB ==0 || !-f $TAX_REFDB[0]){
		...
		if ($doBlasting ==0){ $failedBlastITS .= "Similarity search was not explicitly activated (please use option \"-simBasedTaxo blast\" or \"-simBasedTaxo lambda\").";
		} elsif (!-f $blastBin || !-f $lambdaBin){ $failedBlastITS .= "Neither Lambda nor Blast binary being specified correctly";
		}elsif (@TAX_REFDB ==0 || !-f $TAX_REFDB[0]){$failedBlastITS .= "Reference DB does not exist ($TAX_REFDB[0]).\n";}
		$failedBlastITS .= "\nTherefore LotuS had to abort..\n";
		printL $failedBlastITS,87;
	}
#$doBlasting && (-f $blastBin || -f $lambdaBin) && $TAX_REFDB ne "" && -f $TAX_REFDB)
}
if ($noChimChk <0 || $noChimChk > 3){printL "option \"-deactivateChimeraCheck\" has to be between 0 and 3\n",45;}
if ($saveDemulti <0 || $saveDemulti > 3){printL "option \"-saveDemultiplex\" has to be between 0 and 3\n",46;}
...
if (scalar(@inqArray) > 0 && $numInput != scalar(@inqArray) && -f $inqArray[0]){
...
#die $exec."\n";
if ($ClusterPipe == 0){printL ("Warning: otupipe sequence clustering mode is depreceated.\n\n",0);}
if ($ClusterPipe == 0 && $id_OTU_noise < $id_OTU){printL("id_OTU must be bigger-or-equal than id_OTU_noise\n",2);}
if ($id_OTU> 1 || $id_OTU <0){printL("\"-id\" set to value <0 or >1: $id_OTU\nHas to be between 0 and 1\n",2);}

my $t=$lotus_tempDir;
system("rm -f -r $t") if ($exec==0);
system("mkdir -p $t") unless (-d $t);
if ( ! -d $t ){die("Failed to make tmp dir or doesn't exist: " .$t."\n");} 
#CONSTANT file paths
...
if (!$onlyTaxRedo && !$TaxOnly){
	...
	if ($ClusterPipe  == 0){printL ("Running otupipe $clustMode sequence clustering..\n",0);
	} elsif ($ClusterPipe == 1) {printL ("Running UPARSE $clustMode sequence clustering..\n",0);
	} elsif ($ClusterPipe == 2) {printL ("Running SWARM $clustMode sequence clustering..\n",0);
	} elsif ($ClusterPipe == 3) {printL ("Running CD-HIT $clustMode sequence clustering..\n",0);
	} elsif ($ClusterPipe == 4) {printL ("Running DNACLUST $clustMode sequence clustering..\n",0);}
	...
	system "mkdir -p $newLDir/LotuSLogS/";
	system "mv $highLvlDir $RDP_hierFile $SIM_hierFile $outdir/hierachy_cnt.tax $outdir/cnadjusted_hierachy_cnt.tax $newLDir/";
	system "mv $outdir/LotuSLogS/* $newLDir/LotuSLogS/";
	system("mkdir -p $logDir") unless(-d $logDir);
	if ($extendedLogs){
		system("mkdir -p $extendedLogD") unless (-d $extendedLogs);
	}
...
if ($doBlasting == 1){
	...
	if (!-e $blastBin){printL "Can't find blast binary at $blastBin\n",97;}
} elsif ($doBlasting == 2 )  {
	printL "Similarity search with Lambda\n",0;
	if (!-e $lambdaBin){printL "Can't find LAMBDA binary at $lambdaBin\n",96;}
	if (!-e $lambdaIdxBin){ printL "Can't find valid labmda indexer executable at $lambdaIdxBin\n",98;}
}
...
if ($custContamCheckDB ne ""){
	if (-f $custContamCheckDB){
		...
		printL "Can't find file at location $custContamCheckDB for custom contamination checking..\n",45;
	}
}
printL "--------------------------------------------\n",0;
...
# Pipeline
#////////////////////////// merger ////////////////
my $didMerge=0; my $single1 = "";my $single2 = "";
if (0&& ($platform eq "miseq" || $platform eq "hiSeq") && $numInput == 2){
	...
	if ($exec==0){
		#die $mergCmd."\n";
		printL frame("Merging paired reads $inputArray[0] and $inputArray[2]",0);
		if (!systemL($mergCmd)==0) {printL("Merge command failed: \n$mergCmd\n",3); }
		#printL ($mergCmd."\n"); 
	}
	$didMerge = 1;
}
...
#  cmdArgs["-i_MID_fastq"]
my $sdmcmd="";
my $filterOut = "$t/demulti.fna";my $filterOutAdd = "$t/demulti.add.fna";
if ($numInput>1 ){	$filterOut = "$t/demulti.1.fna,$t/demulti.2.fna"; $filterOutAdd = "$t/demulti.1.add.fna,$t/demulti.2.add.fna";}
my $filOutCmd = "-o_fna ";
if ($UPARSEfilter){
	$filterOut="$t/demulti.fastq"; $filOutCmd = "-o_fastq "; $filterOutAdd="$t/demulti.add.fastq";
	if ($numInput>1){
		$filterOut = "$t/demulti.1.fastq,$t/demulti.2.fastq"; $filterOutAdd = "$t/demulti.1.add.fastq,$t/demulti.2.add.fastq";
	}
}
...
my $derepCmd = ""; my $paired_sdm = "";
my $derepOutHQ = ""; my $derepOutHQ2 = ""; my $derepOutMap = "";

my $sdmDemultiDir = ""; my $sdmOptStr = "-options $sdmOpt";
...
if ($sdmDerepDo){
	...
	$derepOutHQ = "$t/derep.hq.fq"; $derepOutMap = "$t/derep.map";
...
my $demultiSaveCmd = "";
if ($sdmDemultiDir ne ""){
	system "mkdir -p $sdmDemultiDir" unless (-d $sdmDemultiDir);
...
if ($exec==0 && $onlyTaxRedo==0 && $TaxOnly==0){
	...
	if (systemL($sdmcmd) != 0){printL "FAILED sdm demultiplexing step: ".$sdmcmd."\n"; exit(4);} 
	my $fileNum  = `ls -1 $mainSDMlog* | wc -l`;
	system "mkdir -p $logDir/SDMperFile/; mv $mainSDMlog"."0* $logDir/SDMperFile/";
	if ($fileNum > 10){system "tar zcf $logDir/SDMperFile.tar.gz $logDir/SDMperFile/; rm -r $logDir/SDMperFile/";}
	...
	if ($saveDemulti==1 || $saveDemulti==2){#gzip stuff
		...
		system "gzip $sdmDemultiDir/*.fq";
	}
	if ($saveDemulti==1){printL "Demultiplexed intput files with not quality filtering to:\n$sdmDemultiDir\nFinished task, if you want to have a complete LotuS run, change option \"-saveDemultiplex\"\n"; exit(0);}
	if ($saveDemulti==3  && $exec==0 ){#&& $onlyTaxRedo==0){#gzip demultiplexed fastas
		system "mkdir -p $outdir/demultiplexed; ";
		my @allOuts = split /,/,$filterOut;
		foreach (@allOuts){
			$_ =~ m/\/([^\/]+$)/;
			my $fn = $1; my $fns = $fn; $fns =~ s/\.(f[^\.]+)$/\.singl\.$1/;
...
#			system "gzip -c $_* > $outdir/demultiplexed/$fn.gz; rm -f $_*";
			...
			system "gzip -c $fns > $outdir/demultiplexed/$fn.singl.gz; rm -f $_*.singl";
			system "gzip -c $_* > $outdir/demultiplexed/$fn.gz; rm -f $_*";
		}
		@allOuts = split /,/,$filterOutAdd;
		foreach (@allOuts){
			$_ =~ m/\/([^\/]+$)/;
			system "gzip -c $_* > $outdir/demultiplexed/$1.gz; rm -f $_*";
		}
	}
} elsif ($onlyTaxRedo){printL "Skipping Quality Filtering & demultiplexing & dereplication step\n",0;}
my $cmd="";
...
my $A_UCfil; my $tmpOTU = "$t/tmp_otu.fa"; my $OTUfa="$outdir/otus.fa";
...
if ($numInput == 2){
	push(@mergeSeedsFiles,"$t/otu_seeds.1.fq"); push(@mergeSeedsFiles,"$t/otu_seeds.2.fq");
...
if ( $numInput == 2 && @mergeSeedsFiles > 1 && -f $flashBin && $otuRefDB ne "ref_closed"  && $onlyTaxRedo==0 && $TaxOnly==0){
	my $key = "merged";
	$input = "$t/$key";
	my $mergCmd = "";
	if (1&& -f $flashBin){
		...
		$mergCmd = "$flashBin $flOvOption -o $key -d $t -t $BlastCores ".$mergeSeedsFiles[0]." ".$mergeSeedsFiles[1];
		...
		$mergCmd = $usBin . " -fastq_mergepairs ".$mergeSeedsFiles[0]." -reverse ".$mergeSeedsFiles[1]." -fastq_truncqual 12 -fastq_maxdiffs 20 -fastaout $OTUSEED";
		$single1 = "";
	}
	#print $mergCmd."\n";
	if ($exec==0){
	...
	if ($single1 ne ""){#merge not combined reads & check for reverse adaptors/primers
		forceMerge_fq2fna($single1,$single2,$input,$mergeSeedsFiles[0].".singl",$tmpOTU);
	}
	$didMerge = 1;
} elsif ($onlyTaxRedo && $numInput==2){printL "Skipping paired end merging step\n",0;}
...
if ($exec==0 && $onlyTaxRedo==0 && $TaxOnly==0){
	...
	ITSxOTUs($tmpOTU);
	#phiX
	my $phiXcnt = contamination_rem($tmpOTU, $CONT_REFDB_PHIX, "PhiX");
	...
	$OTUrefDBlnk = clean_otu_mat($OTUfa,$OTUrefSEED,$OTUmFile,$OTUrefDBlnk,$phiXcnt,$xtraContCnt) if ($seedExtDone ); #&& $otuRefDB ne "ref_closed" );
	...
	#die ("$tmpOTU, $OTUfa\n");
} elsif ($onlyTaxRedo){printL "Skipping removal of contaminated OTUs step\n",0;}
# ////////////////////////// TAXONOMY ////////////////////////////////////////////
my $RDPTAX=0; 
my $REFTAX=0;
...
if ($doRDPing>0 && $exec==0){ #ITS can't be classified by RDP
	printL frame($msg),0;
	#die "XXX  $cmd\n";
	if (systemL($cmd)){printL "FAILED RDP classifier execution:\n$cmd\n",2; }
	...
	if ($extendedLogs){ system "cp $t/RDPotus.tax $extendedLogD/";}
	if ($RDPTAX > 0){#move confusing files
		if ($extendedLogs){system "mv $outdir/hierachy_cnt.tax $outdir/cnadjusted_hierachy_cnt.tax $extendedLogD/";}
		else {unlink "$outdir/hierachy_cnt.tax"; unlink "$outdir/cnadjusted_hierachy_cnt.tax" if (-e "$outdir/cnadjusted_hierachy_cnt.tax");}
	}
...
if (!$doBlasting && substr($ampliconType,0,3) eq "ITS"){
	...
	$failedBlastITS .= "Blast similarity based annotation is not possible due to: ";
	if (!$doBlasting){ $failedBlastITS .= "Similarity search being deactivated.";
	} elsif (!-f $blastBin || !-f $lambdaBin){ $failedBlastITS .= "Neither Lambda nor Blast binary being specified correctly";
	}elsif (@TAX_REFDB ==0 || !-f $TAX_REFDB[0]){$failedBlastITS .= "Reference DB does not exist (@TAX_REFDB).\n";}
	$failedBlastITS .= "\nTherefore LotuS had to abort..\n";
	printL $failedBlastITS,87;
}
if ($TaxOnly){
...
if(!-d $highLvlDir){
	if(systemL ("mkdir -p $highLvlDir")){printL ("Could not create Higher level abundance matrix directory $highLvlDir.",23);}
}
...
sub ITSxOTUs{
	...
	return if (!$doITSx);
	if (!-e $itsxBin){printL "Did not find ITSx binary at $itsxBin\nNo ITS extraction used\n"; return;}
	if (!-e $hmmsrchBin){printL "Did not find hmmscan binary at $hmmsrchBin\nNo ITS extraction used\n"; return;}
	
	my $outBFile = $otusFA.".itsX";
	my $ITSxReg = "ITS1,ITS2";
	if ($ampliconType eq "ITS2"){$ITSxReg = "ITS2" ; printL "Setting to ITS2 region\n";}
	if ($ampliconType eq "ITS1"){$ITSxReg = "ITS1" ; printL "Setting to ITS1 region\n";}
	my $itsxOrg = "all"; 
	...
	if (scalar(keys(%ITSo)) == 0){printL "Could not find any valid ITS OTU's. Aborting run.\n Remaining OTUs can be found in $outBFile*\n",923;}
...
sub contamination_rem($ $ $ ){
	...
	system "rm -f $outContaminated" if (-e $outContaminated);
	my $contRem = 0;
	#printL frame "Searching for contaminant OTUs with $nameRDB ref DB";
	if ($refDB ne "" && -f $refDB && -s $otusFA){
		#die "hex seq to 50kb pieces\n";
		my $hexDB = $refDB; $hexDB .= ".lts.fna";
		if (0 && -s $refDB > 100000){#hex
			hexFasta($refDB,$hexDB);
			$refDB = $hexDB;
		}  
		my $hitsFile = $otusFA.".cont_hit.uc"; my @hits;
		if ( $VSused==0 ){
			$cmd = "$usBin -usearch_local $otusFA -db $refDB -uc $hitsFile -query_cov .8 -log $logDir/$nameRDB"."_contami_align.log ";
		...
		while (<I>){ my @spl = split(/\t/); if ($spl[0] eq "H"){ push(@hits,$spl[8]) ;$contRem++;} }
		close I;
		#die ("@hits\n");
		#report
		my $hr = readFasta($otusFA); my %OTUs = %{$hr};
		...
		if ($contRem){
			printL frame "Removed $contRem contaminated OTUs ($nameRDB ref DB).\n",0;
		}
...
sub checkXtalk($ $){
	my ($otuFA,$otuM) = @_;
	if (!$doXtalk){return;}
	if ($usearchVer < 11){printL "cannot check for cross-talk, as only implemented in usearch version > 11\n",83;}
	...
	systemL $cmd;
	if (-z "$logDir/crossTalk_analysis.txt"){printL "Cross talk unsuccessful\n";
		system "rm $otuM; mv $otuM1 $otuM;";
	} else {
...
sub clean_otu_mat($ $ $ $ $){
	...
	$hr = readFasta($OTUrefFa); my %refHds = %{$hr};
	my %ORDL = %{$OTUrefDBlnk}; my %ORDL2;
	my $chimRm=0; my $nonChimRm=0; my $chimRdCnt=0; my $emptyOTUcnt=0;
	my %OTUcnt; my %OTUmat;
	#die "$OTUmFile\n$OTUfa\n";
	open I ,"<$OTUmFile"; 
	if ($extendedLogs){open O2x,">$extendedLogD/otu_mat.chim.txt" or printL "Failed to open $extendedLogD/otu_mat.chim.txt",33;}
	while (my $line = <I>){
		$cnt ++;
		chomp $line;
		if ($cnt == 0){$OTUmat{head} = $line;if ($extendedLogs){print O2x $line."\n";}next;}
		my @spl = split(/\t/,$line); my $ot = shift @spl;
	...
	close O; close OF; close OFR;
	if (!$REFflag){unlink "$OTUfa.ref";}
	#die $OTUmFile."\n";
	unlink $OTUmFile; rename "$OTUmFile.tmp", $OTUmFile;
	#writeFasta(\%newOTUs,$OTUfa);
	if ($chimRm){
		my $strTmp = "Removed ". ($chimRm-$phiXCnt-$xtraContCnt)." chimeric\n";
		$strTmp.= "and ".$phiXCnt." phiX contaminant\n" if (1 || $phiXCnt > 0);
		$strTmp.= "and ".$xtraContCnt . " contaminants from custom DB\n" if ($xtraContCnt > 0);
		$strTmp.= "OTUs ($chimRdCnt read counts) from abundance matrix, \n".$nonChimRm." OTUs remaining.\n",0;
...
sub readTaxIn($ $ $ $ ){
	...
	while (my $line = <I>){
		...
		for ( my $i=0;$i<@spl;$i++){$spl[$i] = "?" if ($spl[$i] eq "");}
		next if ($cnt==0 );#|| $spl[0] eq "domain" || $spl[0] eq "Domain" || $spl[0] eq "OTU");
		...
		if ($biomFmt==1){#simple joining of levels
			...
			$Taxo{$spl[$tarEntr]} = join("\", \"",@spl[$taxStart .. ($taxEnd-1)]);
		...
		if ($calcHit2DB){
			if ($spl[$hit2DBEntr] ne "?"){
				...
				if ($markLvl == 1){
				for (my $i=0; $i<@lvls2; $i++){	chomp $spl[$i];	$spl[$i] = $lvls2[$i].$spl[$i];}
				}
				$hit2DBtax{$spl[$hit2DBEntr]} = join("\", \"",@spl[$taxStart .. ($taxEnd-1)]); #in biom format
...
sub biomFmt($ $ $ $ $){
	...
	if (0){#$combineSamples==1){
		open O,">$bioOut.not"; print O "Samples are being combined; biom format is not supported by LotuS in this case\n"; close O;
	...
	my %mapH = %{$mapHref};
	my %combH = %{$combHref};
	if ($hasCombiSmpls){printL "Combined samples in lotus run.. attempting merge of metadata in .biom file\n","w";}
	...
	my %Tax = %{$hr1};  my %hit2db = %{$hr2}; #my @lvls = @{$ar1};
	...
	foreach my $otu (@avOTUs){
		...
		foreach my $smpl (@avSmps){
			...
			if (exists($OTUmat{$smpl}{$otu})){
				...
				#print "$smpl  $otu\n";
				if ($cnt2==1){$biomo.= "0";} else {$biomo.= ",0";} 
			}
		}
		$biomo.="]";
	}
...
sub readPaths_aligners(){
	...
	while (my $line = <I>){
		...
		if ($line =~ m/^usearch\s(\S+)/){$usBin = $1; 
		} elsif ($line =~ m/^vsearch\s+(\S+)/ ){	$VSBin = $1;$VSBinOri = $1; #deactivate default vsearch again.. prob with chimera finder.
		} elsif ($line =~ m/^LCA\s+(\S+)/ ){	$LCABin = $1;
		} elsif ($line =~ m/^multiRDPjar\s+(\S+)/ ){	$mjar = $1;
		} elsif ($line =~ m/^RDPjar\s+(\S+)/ ){	$rdpjar = $1;
		} elsif ($line =~ m/^blastn\s+(\S+)/){$blastBin  = $1;
		} elsif ($line =~ m/^makeBlastDB\s+(\S+)/){	$mkBldbBin = $1;
		} elsif ($line =~ m/^clustalo\s+(\S+)/){	$clustaloBin  = $1;
		} elsif ($line =~ m/^fasttree\s+(\S+)/){	$fasttreeBin = $1;
		} elsif ($line =~ m/^sdm\s+(\S+)/){	$sdmBin = $1;
		} elsif ($line =~ m/^flashBin\s+(\S+)/){	$flashBin = $1;
		} elsif ($line =~ m/^swarm\s+(\S+)/){	$swarmBin =$1;
		} elsif ($line =~ m/^cd-hit\s+(\S+)/){	$cdhitBin = $1;
		} elsif ($line =~ m/^dnaclust\s+(\S+)/){	$dnaclustBin = $1;
		} elsif ($line =~ m/^lambda\s+(\S+)/){	$lambdaBin =$1;
		} elsif ($line =~ m/^lambda_index\s+(\S+)/){	$lambdaIdxBin=$1;
		} elsif ($line =~ m/^itsx\s+(\S+)/){	$itsxBin=$1;
		} elsif ($line =~ m/^hmmsearch\s+(\S+)/){	$hmmsrchBin=$1;
...
sub readPaths(){ #read tax databases and setup correct usage
	...
	die ("$inF does not point to a valid lotus configuration\n") unless (-f $inF);
	open I,"<",$inF;
	my $TAX_REFDB_GG = ""; my $GGinfile = ""; my $TAX_REFDB_SLV = ""; my $SLVinfile = "";
	my $TAX_REFDB_SLV_LSU = ""; my $SLVinfile_LSU = ""; 
	my $UCHIME_REFssu=""; my $UCHIME_REFlsu=""; 
	my $UCHIME_REFits = ""; my $UCHIME_REFits2=""; my $UCHIME_REFits1="";
	my $TAX_RANK_ITS_UNITE=""; my $TAX_REFDB_ITS_UNITE="";
	my  $TAX_UTAX_ITS=""; my $TAX_UTAX_16S="";
	my $TAX_REFDB_16S_HITdb=""; my $TAX_RANK_16S_HITdb = "";
	my $TAX_REFDB_16S_PR2=""; my $TAX_RANK_16S_PR2 = "";
	my $TAX_REFDB_BT = ""; my $TAX_RANK_16S_BT = "";
	while (my $line = <I>){
		chomp $line;
		next if ($line =~ m/^#/);
		next if (length($line) < 5); #skip empty lines
		...
		if ($line =~ m/^UCHIME_REFDB\s+(\S+)/){	$UCHIME_REFssu = $1;
		} elsif ($line =~ m/^UCHIME_REFDB_LSU\s+(\S+)/){	$UCHIME_REFlsu = $1;
		} elsif ($line =~ m/^UCHIME_REFDB_ITS\s+(\S+)/){	$UCHIME_REFits = $1;
		} elsif ($line =~ m/^UCHIME_REFDB_ITS1\s+(\S+)/){	$UCHIME_REFits1 = $1;
		} elsif ($line =~ m/^UCHIME_REFDB_ITS2\s+(\S+)/){	$UCHIME_REFits2 = $1;
		} elsif ($line =~ m/^TAX_REFDB_GG\s+(\S+)/){	$TAX_REFDB_GG = $1;
		} elsif ($line =~ m/^TAX_RANK_GG\s+(\S+)/){	$GGinfile=$1;
		} elsif ($line =~ m/^TAX_REFDB_SSU_SLV\s+(\S+)/ || $line =~ m/^TAX_REFDB_SLV\s+(\S+)/){	$TAX_REFDB_SLV = $1;
		} elsif ($line =~ m/^TAX_RANK_SSU_SLV\s+(\S+)/ || $line =~ m/^TAX_RANK_SLV\s+(\S+)/){	$SLVinfile=$1;
		} elsif ($line =~ m/^TAX_REFDB_LSU_SLV\s+(\S+)/){	$TAX_REFDB_SLV_LSU = $1;
		} elsif ($line =~ m/^TAX_RANK_LSU_SLV\s+(\S+)/){	$SLVinfile_LSU=$1;
		} elsif ($line =~ m/^TAX_REFDB_ITS_UNITE\s+(\S+)/){	$TAX_REFDB_ITS_UNITE = $1;
		} elsif ($line =~ m/^TAX_RANK_ITS_UNITE\s+(\S+)/){	$TAX_RANK_ITS_UNITE=$1;
		#HITdb
		} elsif ($line =~ m/^TAX_RANK_HITdb\s+(\S+)/){	$TAX_RANK_16S_HITdb=$1;
		} elsif ($line =~ m/^TAX_REFDB_HITdb\s+(\S+)/){	$TAX_REFDB_16S_HITdb=$1;
		#PR2
		} elsif ($line =~ m/^TAX_RANK_PR2\s+(\S+)/){	$TAX_RANK_16S_PR2=$1;
		} elsif ($line =~ m/^TAX_REFDB_PR2\s+(\S+)/){	$TAX_REFDB_16S_PR2=$1;
		#bee tax
		} elsif ($line =~ m/^TAX_RANK_BEE\s+(\S+)/){	$TAX_RANK_16S_BT=$1;
		} elsif ($line =~ m/^TAX_REFDB_BEE\s+(\S+)/){	$TAX_REFDB_BT=$1;
		#UTAX
		} elsif ($line =~ m/^TAX_REFDB_ITS_UTAX\s+(\S+)/){	$TAX_UTAX_ITS = $1;
		} elsif ($line =~ m/^TAX_REFDB_SSU_UTAX\s+(\S+)/){	$TAX_UTAX_16S=$1;
		#ref DB for PhiX conatminations
		} elsif ($line =~ m/^REFDB_PHIX\s+(\S+)/ ){	$CONT_REFDB_PHIX = $1;
			...
			if (scalar(@spl)==7){
				for (my $i=0; $i<@spl; $i++){
					if ($spl[$i] > 100 || $spl[$i] < 0){print "Error reading \"IdentityThresholds\" from configuration file: every entry has to be <100 and >0. Found ".$spl[$i].".\n"; exit(22);}
					$idThr[$i] = $spl[$i];
				}
			} else {
	...
	if ($doBlasting  > 0){
		...
		if ($doBlasting == 3){
			$refDBwanted="UTAX";
			if ($usearchVer < 8.1){printL "Your usearch version ($usearchVer) is not compatible with utax, please upgrade: http://www.drive5.com/usearch/\n",63;}
			if ($usearchVer >= 9){printL "Please use usearch ver 8 for now, if you want to use utax\n",22;}
		}			
		elsif ($defDBset){ if (!-f $TAX_REFDB_GG && $refDBwanted eq "GG"){$refDBwanted="SLV";} }
		for my $subrdbw (split /,/,$refDBwanted){
			...
			if ($subrdbw =~ m/^UNITE$/ || substr($ampliconType,0,3) eq "ITS"){ #overrides everything else
					...
					push @TAX_REFDB , $TAX_REFDB_ITS_UNITE; push @TAX_RANKS , $TAX_RANK_ITS_UNITE;
				if (!-f $TAX_REFDB[-1] || !-f $TAX_RANKS[-1]){printL "Could not find UNITE ITS DB files at \n$TAX_REFDB[-1]\n$TAX_RANKS[-1]\nPlease check that these files exist.\n",55;}
			...
			if ($subrdbw =~  m/^HITDB$/){
				push @TAX_REFDB , $TAX_REFDB_16S_HITdb; push @TAX_RANKS , $TAX_RANK_16S_HITdb;
				push @refDBname , "HITdb";
				if (!-f $TAX_REFDB[-1] || !-f $TAX_RANKS[-1]){printL "Could not find HITdb files at \n$TAX_REFDB[-1]\n$TAX_RANKS[-1]\nPlease check that these files exist.\n",55;}
			...
			if ($subrdbw =~  m/^PR2$/){
				push @TAX_REFDB ,$TAX_REFDB_16S_PR2; push @TAX_RANKS , $TAX_RANK_16S_PR2;
				if (!-f $TAX_REFDB[-1] || !-f $TAX_RANKS[-1]){printL "Could not find PR2 files at \n$TAX_REFDB[-1]\n$TAX_RANKS[-1]\nPlease check that these files exist.\n",55;}
			...
			if ($subrdbw =~ m/^GG$/){
				if (!-f $TAX_REFDB_GG || !-f $GGinfile){printL "Could not find greengenes DB files at \n$TAX_REFDB_GG\n$GGinfile\nPlease check that these files exist.\n",55;}
				...
				push @TAX_REFDB , $TAX_REFDB_GG; push @TAX_RANKS , $GGinfile;
			...
			if ($subrdbw =~ m/^BEETAX$/){
				...
				push @TAX_REFDB , $TAX_REFDB_BT; push @TAX_RANKS , $TAX_RANK_16S_BT;
			...
			if ($subrdbw =~ m/^SLV$/ ){
				...
				if ($ampliconType eq "SSU"){
					...
					push @TAX_REFDB , $TAX_REFDB_SLV; push @TAX_RANKS , $SLVinfile;
				}elsif ($ampliconType eq "LSU"){
					printL "Using Silva LSU ref seq database.\n",0;
					push @TAX_REFDB , $TAX_REFDB_SLV_LSU; push @TAX_RANKS , $SLVinfile_LSU;
				}
				if (!-f $TAX_REFDB[-1] || !-f $TAX_RANKS[-1]){printL "Could not find Silva DB files at \n$TAX_REFDB[-1]\n$TAX_RANKS[-1]\nPlease check that these files exist.\n",55;}
	...
	if (! -f $VSBin || $preferVsearch==0){
		$VSBin = $usBin; $VSused = 0;
		finWarn("Could not find vsearch binaries at\n$VSBin\n, switching to usearch binaries instead\n") if (! -f $VSBin);
	} else {
		$citations .= "VSEARCH 1.13 (chimera de novo / ref; OTU alignments): Rognes T (2015) https://github.com/torognes/vsearch\n";
	}
...
sub help {
   ...
   print "  -barcode [file path to fastq formated file with barcodes (this is a processed mi/hiSeq format)]\n";
   ...
   print "  -threads [number of threads to be used, default 1]\n  -UP [(1) use UPARSE, (0) use otupipe, default 1]\n";
   ...
   print "  -custContamCheckDB [Default: empty. This option checks in analogy to the phiX filter step in a custom DB (e.g. mouse genome, needs to be fasta format), for contaminant OTUs that are more likely to derrive from this genome than e.g. bacteria. Example: -custContamCheckDB /YY/mouse.fna]\n";
   ...
   print "  -TaxOnly skip most of the lotus pipeline and only run a taxonomic classification on a fasta file (provided via \"-i\" (could be an OTU fasta).\n";
   ...
   print "  -simBasedTaxo [(previously doBlast) 0: deavtivated (just use RDP); [1 or \"blast\"]: use Blast; [2 or \"lambda\"]: use LAMBDA to search against a 16S reference database for taxonomic profiling of OTUs; [3 or \"utax\"]: use UTAX with custom databases. Default 0]\n";
   ...
   print "Databases can be combined, with the first having the highest prioirty. E.g. \"PR2,SLV\" would first use PR2 to assign OTUs and all unaasigned OTUs would be searched for with SILVA, given that \"-amplicon_type LSU\" was set.\n";
   ...
   print "  -derepMin [minimum size of dereplicated clustered, one form of noise removal. Can be complex terms like \"10:1,3:3\" -> meaning at least 10x in 1 sample or 3x in 3 different samples. Default 1]\n";
   print "Can also be a custom fasta formatted database: in this case provide the path to the fasta file as well as the path to the taxonomy for the sequences using -tax4refDB. See also online help on how to create a custom DB.]\n";
   print "  -count_chimeras [T: count chimeric reads into their estimated original OTUs, F: do nothing. Default F]\n";
...
sub get16Sstrand($ $){#
	...
	if(systemL($cmd) != 0){printL "FAILED pre-run blast command:\n$cmd\n",5;}
	#die "$t\n$cmd\n";
...
sub calcHighTax($ $ $ $ $){
	...
	my $getHit2DB=0; $getHit2DB=1 if ($xtraOut ne "");
	my ($hr1,$ar1,$hr2,$tax4RefHR) = readTaxIn($inT,$LCAtax,0,$getHit2DB);	
	my %Taxo = %{$hr1}; my @lvls = @{$ar1};my %hit2db = %{$hr2};
	my %fails = %{$failsR};
	my %OTUmat=%{$hr};
	my $SEP = "\t";
	my @avSmps = sort(keys %OTUmat);
	...
	my $cnt=0; my %totCnt; my %assCnt; my %assTax; my %unassTax;
	foreach my $l (@lvls){
		$cnt++; next if ($cnt==1); #domain doesn't need a file..
		my $lvlSmplCnt=0; my $assignedCnt =0;my $taxAssingedCnt=0; my $taxUnAssingedCnt = 0;
		my $isUnassigned=0;
		...
		foreach my $ta (@avTax){
			print O "\n".$ta;
			if ($ta =~ m/\?$/) {
				$isUnassigned = 1 ; $taxUnAssingedCnt ++;
			} else {$taxAssingedCnt++;}
			my @OTUli = @{$Taxo{$l}{$ta}};
			#die(join("-",@OTUli)." ".$ta."\n");
			foreach my $sm (@avSmps){
		...
		close O;
		$totCnt{$l} = $lvlSmplCnt; $assCnt{$l} = $assignedCnt; $assTax{$l}=$taxAssingedCnt; $unassTax{$l} = $taxUnAssingedCnt;
...
#print some stats
	...
	foreach my $l (@lvls){
		$cnt++; next if ($cnt==1); #domain doesn't need a file..
		if ($cnt == 2){printL "Total reads in matrix: $totCnt{$l}\nTaxLvl	%AssignedReads	%AssignedTax\n",0;}
		my $assPerc = 0;$assPerc = $assCnt{$l}/$totCnt{$l} if ($totCnt{$l}>0);
		my $assPerc2 = 0;$assPerc2 = $assTax{$l}/($assTax{$l}+$unassTax{$l}) if ( ($totCnt{$l} + $unassTax{$l}) >0 );
...
sub buildTree($ $){
	...
	if (($exec==0 && $onlyTaxRedo==0 && -f $clustaloBin) || ($exec==0 && $onlyTaxRedo==0 && -f $fasttreeBin)){
		$citations .= "======== Phylogenetic tree building ========\n";
	}
	
	if ($exec==0 && $onlyTaxRedo==0 && -f $clustaloBin){
		if (!-f $OTUfa){printL "Could not find OTU sequence file:\n$OTUfa\n",5; }
		if (system($cmd) != 0){printL "Fallback to single core clustalomega\n",0; $cmd = $clustaloBin." -i $OTUfa -o $multAli --outfmt=fasta --threads=1 --force\n";}
		if (system($cmd) != 0){printL "FAILED multiple alignment command: ".$cmd."\n" ,5;}
	...
	if ($exec==0 && $onlyTaxRedo==0 && -f $fasttreeBin){
		if (!-f $multAli){printL "Could not find multiple alignment file:\n$multAli\n",5;}
		$cmd = $fasttreeBin." -nt -gtr -no2nd -spr 4 -log $logDir/fasttree.log -quiet $multAli > $outTree";
		#die($cmd."\n");
		if ($exec==0){
			printL"Building tree..\n";
			if (system($cmd) != 0){printL "FAILED tree building command: ".$cmd."\n",5;}
...
sub rework_tmpLines($ $ $ $ $ $ $ $){
	my ($tmpLinesAR,$hr1,$sotu,$splAR,$sID,$sLength,$GGhr,$maxGGdep) = @_;
...
sub getTaxForOTUfromRefBlast($ $ $){
	...
	while (my $line = <B>){
		...
		if ($sotu eq $totu){
			...
			#print "Maybe\n";
			($retRef) = rework_tmpLines(\@tmpLines,$retRef,$sotu,\@spl,$sID,$sLength,\%GG,$maxGGdep) if ($sotu ne "");
	...
	if ($sotu ne ""){
		my @spl = split("\t",$line2);
		($retRef) = rework_tmpLines(\@tmpLines,$retRef,$sotu,\@spl,$sID,$sLength,\%GG,$maxGGdep);
	}
	close B;
	#debug 
...
sub newUCSizes($ $){
	...
	while (my $line = <UC>){
		...
		if (!exists($fa{$tok})){die("Expected fasta seq with head $tok\n");}
		$fa{$tok} += $cnt;
	}
	close UC;
...
sub readFasta($){
	...
	my $temp; my $line; 
	my $trHe =<FAS>; chomp ($trHe);  $trHe = substr($trHe,1);$trHe =~ s/;size=\d+;.*//; $trHe =~ s/\s.*//;
	while($line = <FAS>){
		if ($line =~ m/^>/){
			#finish old fas`
			$Hseq{$trHe} = $temp; 
			#prep new entry
			chomp($line);
			$trHe = substr($line,1); $trHe =~ s/;size=\d+;.*//; $trHe =~ s/\s.*//;
			$temp = "";
			next;
		}
		$temp .= ($line);
...
sub swarm4us_size($){
	...
	close I; close O;
#	die $otuf."\n";
	system "rm $otuf;mv $otuf.ttmp $otuf"; 
}
sub replaceFastaHd($ $){
	my ($href,$ifil) = @_;
	my $ofil = $ifil.".tmp";
...
sub writeUTAXhiera($ $ $){
	...
	my %fails = %{$failsR}; my @avOTUs = @{$avOTUsR};
...
sub writeBlastHiera($ $ $){
	...
	#
	foreach (keys(%fails)){
		if (exists( ${$taxr}{$_}) &&  ${$taxr}{$_} && maxTax(${$taxr}{$_}) > 0 ){
			delete $fails{$_};
		} else {
			print H $_."\t?\t?\t?\t?\t?\t?\t?"."\n";
		}
...
sub checkLtsVer($){
	...
	if (! head($url)) {printL "LotuS server seems to be down!\n"; return $sdmVer;}
	my $updtmpf = get($url);

	my $msg = ""; my $hadMsg=0; my $msgCont="";
	...
	if ($hadMsg || $updates ne ""){
		printL "New LotuS updates discovered (10s wait).\nIf you want to install updates\n  (1) Press \"Ctrl-c\"\n  (2) perl autoInstall.pl\n\n";
		printL "To deactivate, open $sdmOpt and change \"CheckForUpdates\" to \"0\"\n";
		sleep(10);
	}
 ...
 sub readMap(){
	 ...
	 my %mapH; my %combH;
	 unless (open M,"<",$map) {printL( "Couldn't open map file $map: $!\n",3);}
	 local $/=undef;
	 my $mapst=<M>;
	 $mapst =~ s/\r\n|\n|\r/\n/g;
	 close M;
	...
	my $cnt=0; my $warnTrig =0 ; my $fileCol = -1; my $twoFiles = 0; my $CombineCol = -1;
	my $hasCombiSmpl = 0; my $colCnt=-1;
	 foreach my $line (@mapar){
		$cnt++;
		next if ($cnt > 1 && ($line =~ m/^#/ || length($line) < 2));
		chomp($line); my @spl = split("\t",$line);
		if (@spl==0){
			$warnTrig=1;finWarn("Mapping file contains lines that cannot be split by tab seperator (line $cnt):\"$line\"\n");
			next;
		}
		...
		chomp($spl[0]);
		if ($spl[0] eq ""){
			$warnTrig=1;finWarn( "Empty SampleID in row number $cnt, skipping entry.\n");
			next;
		}
		my $smplNms = $spl[0];
		if ($fileCol != -1){
		...
		if ($cnt == 1){
			...
			foreach (@spl){
				if ($_ eq "fastqFile" || $_ eq "fnaFile"){
					...
					if ($fileCol != -1 ){printL "both fastqFile and fnaFile are given in mapping file, is this intended?\n";}
					$fileCol = $ccn;
				}
		...
		if ($line =~ m/\"/){$warnTrig=1;finWarn("Possible biom incompatibility: Mapping file contains \" for sample $smplNms. Lotus is removing this.");}
		if ($line =~ m/ /){$warnTrig=1;finWarn("Possible biom incompatibility: Mapping file contains spaces for sample $smplNms");}
		if ($line =~ m/[^\x00-\x7F]/){$warnTrig=1;finWarn("Possible biom incompatibility: Mapping file contains non-ASCII character for sample $smplNms");}
		$line =~ s/\s+/\t/g;
		if ($smplNms =~ m/^\s/ || $smplNms =~ m/\s$/){
			printL "SampleID $smplNms contains spaces. Aborting LotuS as this will lead to errors, please fix.\n",5; 
		}
		if ($cnt==1 ){#col names
			if ( $line =~ m/\t\t/){printL "Empty column header in mapping file:\n check for double tab chars in line 1:\n$map\n",4;}
			if ($smplNms ne '#SampleID'){printL "Missing \'#SampleID\' in first line of file $map\n Aborting..\n",65;}
			#if ($line =~/\tCombineSamples\t/){$combineSamples=1;} #deprecated
			my $hcnt =0;
			foreach (@spl){ 
				$hcnt++;
				if ( m/^\s*$/ ){printL "Empty column header in mapping file (column $hcnt)\n$map\n",4;}
			}
			if ( $line =~ m/\t\t/){printL "Empty header in mapping file:\n check for double tab chars in line 1:\n$map\n",4;}
		}
		splice(@spl,0,1);
		#print $smplNms." ".$spl[0]."\n";
		for (my $i=0;$i<@spl;$i++){
	 ...
	 #print keys %mapH."\n";
	 if (scalar( keys %mapH) ==0){printL("Could not find sample names in mapping file (*nix/win file ending problem?\n",9);}
...
sub utaxTaxAssign($ $){
	...
	$cmd = "";
	unless (-e $utUdb) {$cmd .= "$usBin -makeudb_utax $utFas -output $utUdb -taxconfsin $utConf\n";}
...
#die "$doBlasting\n";
	...
	if ($doBlasting == 1 ){
		...
		unless (-f $DB.".nhr"){
			if (systemL($cmd)){printL "makeBlastDB command failed:\n$cmd\n",3;}
		}
		my $strand = "both";
		...
		if (!-s $query){$cmd = "touch $taxblastf";} else {
		...
		if (!-f $udbDB){
			...
			if (system("$usBin -makeudb_ublast $DB -wordlength 14 -output $udbDB") != 0){ die ("make udb command failed\n");}
		}
		...
		if ( $lamVer > 0.4 && -f $DB.".dna5.fm.sa.val" && !-f  "$DB.dna5.fm.lf.drv.wtc.24") {
		...
		if (!-f $DB.".dna5.fm.sa.val"  ) {
			...
			if (system($cmdIdx)){printL("Lamdba ref DB build failed\n$cmdIdx\n",3);}
		}
		...
		if (!-s $query){$cmd = "rm -f $taxblastf;touch $taxblastf\n";} else{
			$citations .= "Lambda taxonomic similarity search: Hauswedell H, Singer J, Reinert K. 2014. Lambda: the local aligner for massive biological data. Bioinformatics 30: i349-i355\n" unless ($citations =~ m/Lambda taxonomic similarity search:/);
		}
		$simMethod = "LAMBDA";
		#die $cmd."\n";
	} else {
	...
	if ($clQue ne ""){
		...
		my $qcmd = "#!/bin/bash\n#\$ -cwd\n#\$ -q $clQue\n\n";
		open Q,">",$tq;		print Q $qcmd.$cmd;		close Q;
		if ($exec==0){
			printL frame("Assigning taxonomy against reference using blast\nelapsed time: $duration s"),0;
			$cmd  ="qsub -sync yes -pe make $BlastCores -N BLASTdead $tq";
		if(systemL($cmd)){printL "Submission of blast command to cluster failed:\n$cmd\n",3; }
		}
	} else {
		if ($exec==0){
		printL frame("Assigning taxonomy against reference using $simMethod\nelapsed time: $duration s");
		#print $cmd."\n";
		if (systemL($cmd) ){printL "$simMethod against ref database failed:\n$cmd\n",3;}
		}
	}
	#die $cmd."\n";
	return $taxblastf;
...
sub assignTaxOnly($ $){
	...
	my $blastFlag = $doBlasting != 3 && $doBlasting && (-f $blastBin || -f $lambdaBin) ;#set that blast has to be done
	 
	my $fullBlastTaxR = {};my $GGloaded = 0;  my %GG; 
	if ($doBlasting==3 ){
		...
		for (my $DBi=0;$DBi<@TAX_REFDB; $DBi++){
			...
			my $DB = $TAX_REFDB[$DBi]; my $DBtax = $TAX_RANKS[$DBi];
		...
		#die $cmd;
		if (system $cmd){printL "LCA command $cmd failed\n",44;}
		system "cp @blOuts $output\n";
	} 
}
...
sub makeAbundTable2($ $){
	...
	if ($REFflag){
		#$OTUrefSEED
		if (!$GGloaded){%GG = getGGtaxo($TAX_RANKS[0],$refDBname[0]) ; $GGloaded = 1;}
	...
	if ($doBlasting==3 ){
		...
		if ($blastFlag){ #do blast taxnomy 
			...
			for (my $DBi=0;$DBi<@TAX_REFDB; $DBi++){
				...
				#---------  single core way --------------
				%GG = getGGtaxo($DBtax,$refDBname[$DBi]) ; $GGloaded = 1;
	...
	#	my @t = @{${$fullBlastTaxR}{"OTU_1"}}; die "@t\n";
		undef $BlastTaxR; undef $fullBlastTaxR;
...
while (my $line = <UC>){
	...
	my $hit = $spl[9];
	my $que = $spl[8];
	my $hitsize=1; my $quesize=1;
	if ($mode >= 1){
		 # if $mode==1; 
		if ($mode==1){$que =~ s/;size=(\d+).*$//; $quesize = $1;}
		$hit =~ s/;size=(\d+).*$//;$hitsize=$1;
	}
	if ($mode==3){ #switch hit and que, because in UPARSE case, reads are aligned to OTUs (= not backtracing)
		#print($quesize." ".$hitsize."\n");
		my $temp = $hit; $hit=$que; $que = $temp;
		$temp = $hitsize; $hitsize = $quesize; $quesize = $temp;
	}
...
#	print $hit." ".$que."\n"; die();
	...
	if ($mode!=2){
	if ( $spl[0] eq "C"){#all "H" should have appeared at this point; cross-check if cluster size is right
		...
		if (!exists($clus{$que} )){
			...
			if ($spl[2] != $expSize{$que} ){
				printL("$que :: expected ".$spl[2]." cluster size. found cluster size: ".$expSize{$que} ." from array with S $arraySize.\n",0);
				foreach (@{$clus{$que}}){
					printL $_."\n",0;
				}
				die();
...
my $cmd = $cdh454." -i $input -o $t/OTUs.fa -c .$id_OTU -M 0 -T $BlastCores";
print $cmd."\n";
my $OTUfa="$t/OTUs.fa";
...
my $cmd ="$VSBin -derep_fulllength $filterOut -output $t/derep.fa -uc ".$UCguide[0]." -threads $uthreads -log $logDir/derep.log -sizeout";
if (systemL($cmd) != 0){exit(1);}

systemL("ls -lh $t/derep.fa");
printL("\n =========================================================================\n Cluster for err. corr. at ". 100*$id_OTU_noise." pct, output consensus to cons.fa.\n =========================================================================\n",0);
#denoising step
#print_nseq("$t/derep.fa");
$cmd="$VSBin -sortbylength  $t/derep.fa -output $t/derep.fas -minsize $dereplicate_minsize -log $logDir/sortbylen_pre.log";
if (systemL($cmd) != 0){exit(1);}
$cmd = "$VSBin -cluster_smallmem  $t/derep.fas -centroids  $t/cons.fa -uc ".$UCguide[2]." -id $id_OTU_noise -sizein -sizeout -log $logDir/cluster_err.log";
if (systemL($cmd) != 0){exit(1);}
...
$cmd="$VSBin -sortbysize $t/cons.fa -output $t/cons.fas -minsize $dereplicate_minsize -log $logDir/sortsize1.log";
if (systemL($cmd) != 0){exit(1);}
...
$cmd="$VSBin -uchime_denovo  $t/cons.fas -chimeras $t/chimeras_denovo.fa -nonchimeras $t/tmp1.fa -abskew $chimera_absskew -log $logDir/uchimedn.log";
if (systemL($cmd) != 0){exit(1);}
...
if ( $UCHIME_REFDB ne "" && -f $UCHIME_REFDB){
	...
	$cmd = "$VSBin -uchime_ref  $t/tmp1.fa -db $UCHIME_REFDB -strand plus -chimeras $t/chimeras_ref.fa -nonchimeras $t/tmp2.fa -threads $uthreads -log $logDir/uchimedb.log";
	if (systemL($cmd) != 0){exit(1);}
	systemL("ls -lh $t/tmp2.fa");

	#print_nseq("$t/tmp2.fa");
	systemL("cat $t/chimeras_denovo.fa $t/chimeras_ref.fa > $t/all_chimers.fa");
} else {
	systemL("cat $t/chimeras_denovo.fa > $t/all_chimers.fa");
}

printL("=========================================================================\n \"Sort chimeric sequences by abundance, output tmp2c.fa\"\n",0);
$cmd = "$VSBin -sortbysize $t/all_chimers.fa -output $t/tmp2c.fas -log $logDir/sort2c.log";
if (systemL($cmd) != 0){exit(1);}
systemL("ls -lh $t/tmp2c.fas");

printL("\n =========================================================================\n Cluster chimeras at ". 100*$id_OTU." pct, output in tmp3c.fa\n =========================================================================\n",0);
$cmd="$VSBin -cluster_smallmem  $t/tmp2c.fas -id $id_OTU -centroids $t/tmp3c.fa -sizein -sizeout -maxrejects 64 -log $logDir/cluster2c.log";
if (systemL($cmd) != 0){exit(1);}
#system("ls -lh $t/tmp2c.fas");

printL("\n =========================================================================\n Assign sequential numbers to chimeric OTUs\n =========================================================================",0);
system("$sdmBin -i_fna $t/tmp3c.fa -number T -prefix ${chimera_prefix} -o_fna $outdir/chimeras.fa");
...
$cmd="$VSBin -sortbysize $t/tmp2.fa -output $t/tmp2.fas -minsize $dereplicate_minsize  -log $logDir/sort2.log";
if (system($cmd) != 0){exit(1);}
system("ls -lh $t/tmp2.fas");

printL("\n =========================================================================\n Cluster at ". 100*$id_OTU." to merge OTUs, output in tmp3.fa\n =========================================================================",0);
my $xcmd="";
if ($existing_otus ne ""){
	$xcmd="-db $existing_otus";
}
$cmd = "$VSBin -cluster_smallmem  $t/tmp2.fas -id $id_OTU -uc ".$UCguide[4]." -centroids $outfile -sizein -sizeout -maxrejects 64 $xcmd -log $logDir/cluster2.log";
if (system($cmd) != 0){exit(1);}
print_nseq("$outfile");

system("cat $existing_otus $outdir/otus.fa $outdir/chimeras.fa > $t/db.fa");
...
sub splitFastas($ $){
	...
	open my $out,">$inF.$fCnt" or printL "Fatal: Can't open output fasta file $inF.$fCnt\n",59; 
	while (my $l = <I>){
		if ($l =~ m/^>/){
			$curCnt++;
			if ($curCnt > $pPerFile){
...
sub combine($ $){
	...
	if( $osname eq 'MSWin32' ){
		system("type $fadd  $freal > $freal");
	} else {
		system("cat $fadd >> $freal");
	}
	unlink("$fadd");
}
sub usearchDerepSort($ ){
	...
	#die($cmd);
	if (system($cmd) != 0){printL "Failed dereplication\n",1;}

	my $dcnt=1; my $bigDerepCnt=0;
	my @lotsFiles = ($filtered); my @subDerepFa = ("$t/derep.fa"); #my @subDerepUC = ("$t/derep.uc");
	my @subDerepFaTmp=();
	while (-f $filtered.".".$dcnt){
		if ($dcnt==1){@subDerepFaTmp = @subDerepFa; @subDerepFa = ();}
		push(@lotsFiles,$filtered.".".$dcnt);
		push(@subDerepFaTmp,"$t/derep.fa.$dcnt"); #push(@subDerepUC,"$t/derep.uc.$dcnt");
		$cmd ="$VSBin -derep_fulllength $filtered.$dcnt $outputLab $subDerepFaTmp[-1] -sizeout -threads $uthreads"; #-uc $t/derep.uc -log $logDir/derep.log
		#die $cmd;
		if (system($cmd) != 0){printL "Failed dereplication\n",1;}
		...
		if (($dcnt+1) % 10 == 0 || !-f $filtered.".".($dcnt+1)){ #intermediary merge.. too big otherwise
			my $firstF = shift(@subDerepFaTmp);
			#print join(" ",@subDerepFaTmp)."\n";
			my $pid = open(CMD,"| cat ".join(" ",@subDerepFaTmp)." >> $firstF") || printL "Merge of derep subfiles failed.\n",1;
			defined($pid) || printL ("Cannot fork derep of subfiles.\n",1);
			close CMD;
			foreach (@subDerepFaTmp){unlink;}@subDerepFaTmp = ();
			...
			if (system($cmd) != 0){printL "Failed intermediate dereplication\n",1;}
			...
			if (@subDerepFa > 1){
				...
				$pid = open (CMD,"| cat ".join(" ",@subDerepFa)." >> $firstF");
				defined($pid) || printL ("Cannot fork derep of subfiles.\n",1);
	...
	$cmd="$VSBin -sortbysize  $t/derep.fa $outputLab $t/derep.fas -minsize $dereplicate_minsizeX -log $logDir/sortbysize.log";
	if (system($cmd) != 0){exit(1);}
	unlink("$t/derep.fa");
	return("$t/derep.fas");
}
...
sub removeNonValidCDhits($ $ $){
	my ($refDB4otus, $addon, $denNN) = @_;#idea is to use the addon.clstr to find refSeqs that were used the denNN file (are not in there)
...
sub dnaClust2UC($ $){
	my ($in,$out) = @_;
	print "DNAclust1";
	my %cluster; my %clSize;
	my %clus_denovo; my %clDNSize;
	...
	while (my $line = <I>){
		...
		die "DNAclust:: double Cluster detected: $curCl\n" if (exists($clus_denovo{$curCl}) || exists($clus_denovo{$curCl}));
	...
	#print "uc\n";
	open O,">$out.ref" or printL "Fatal: Can't open dnaclust uc rewrite $in\n",37;
	foreach my $k (@refs){
		my @mem = @{$cluster{$k}};
		my $ref = $k;#.";size=".@mem.";";
...
if (!$sdmDerepDo){
	...
	if (!-f $derepl || -z $derepl){printL "The sdm dereplicated output file was either empty or not existing, aborting lotus.\n$derepl\n",1;}
...
if ($ClusterPipe ==1 ){
	my $maxhot = 62; my $maxdrop = 12;
	#too many files need a more thorough clustering process
	if ($totSeqs> 12000000 && $totSeqs<24000000){
		$maxhot = 72; $maxdrop = 15;
	} elsif ($totSeqs>=24000000){
		$maxhot = 92; $maxdrop = 18;
	}
	my $id_OTUup = $id_OTU;
	my $outputLab = "-output"; my $idLabel = "-id";
	my $xtraOptions = "";
	if ($usearchVer >= 10){#just to control id percentage
		$idLabel = ""; 
		$id_OTUup = "";
		if ($id_OTU != 0.97){printL "UPARSE 10 does only support 97% id OTU clusters\n","w";}
	}elsif ($usearchVer >= 8){
		$idLabel = "-otu_radius_pct"; 
		$id_OTUup = 100-($id_OTU * 100);
		if ($id_OTUup < 0 || $id_OTUup > 50){printL "UPARSE cluster radius $id_OTUup not valid, aborting..\n",54;}
	}
	if ($usearchVer >= 8){
	...
	if ($REFflag){#$otuRefDB eq "ref_closed" || $otuRefDB eq "ref_open"){
		...
		$cmd = "$cdhitBin -T $uthreads -o $OTUfastaTmp -c $id_OTU -M 0 -i $derepl -n 9 -g 1";#-aL 0.77 -aS 0.98 
	}
...
if ($ClusterPipe ==4 ){ #transcribe dnaclust out to .uc
	($refsCL,$refCLSiz,$denovos,$denoSize) = dnaClust2UC($dnaclustOut,$UCguide[0]);
}
if ($REFflag){
...
#do in later loop
#print "ASD\n$noChimChk\n$OTUfastaTmp\n$ClusterPipe\n";
if ($ClusterPipe != 1 && -e $OTUfastaTmp && ($noChimChk ==0 || $noChimChk == 3)){ #not uparse && actual reads in fasta
	...
	if (systemL($cmd) != 0){printL("uchime de novo failed// aborting\n",1);}
	if ($usearchVer >= 9){
		$citations .= "uchime2 chimera detection deNovo: Edgar, R.C. (2016), UCHIME2: Improved chimera detection for amplicon sequences, http://dx.doi.org/10.1101/074252..\n";
	} elsif ($VSused){
...
if ($sdmDerepDo){
	...
	if ( ($usearchVer < 8  && $ClusterPipe == 1 ) 
			|| $ClusterPipe == 2 || $ClusterPipe == 3 ) { #usearch8 has .up output instead
		$cmd = "$VSBin -usearch_global ".$derepl." -db $outfile -uc $UCguide[0] $userachDffOpt $vsearchSpcfcOpt";#-threads  $BlastCores";
		#die $cmd."\n";
		if (system($cmd) != 0){printL("vsearch backmapping command aborted:\n$cmd\n",1);}
	}
...
my @lotsFiles = ($filterOutAdd); my $dcnt=1; while (-f $filterOutAdd.".".$dcnt && $dcnt < 100000){	push(@lotsFiles,$filterOutAdd.".".$dcnt);$dcnt++}
...
foreach my $subF (@lotsFiles){
	...
	if (-f $subF && !-z $subF){
		...
		open T,"<",$subF;		my $lcnt = 0;		while (<T>){$lcnt ++;last if ($lcnt > 5);}		close T;
		if ($lcnt > 2){ #file contains reads, so map
			...
			if (-s $OTUfastaTmp){
				if (system($cmd) != 0){printL ("Failed: $cmd\n",1); }
			} else { system("touch $tmpUC"); }
			system("cat $tmpUC >> ".$UCguide[0].".ADD");	unlink $tmpUC;
			...
			if (($REFflag) && -s "$OTUfastaTmp.ref"){
				...
				if (system($cmd) != 0){printL ("Failed: $cmd\n",1); }
				system("cat $tmpUC >> ".$UCguide[0].".ADDREF");unlink $tmpUC;
			}
		}
	}
}
...
if (-e $derepl.".rest" && -s $derepl.".rest"){
	...
	if (-s $OTUfastaTmp){
		if (system($cmd) != 0){exit(1);}
	} else {system("touch $restUC");}
	system("cat $restUC >> ".$UCguide[0].".REST");	unlink $restUC;
	if (($REFflag)  && -s "$OTUfastaTmp.ref"){
		$cmd = "$VSBin -usearch_global ".$derepl.".rest"." -db $OTUfastaTmp.ref -uc ".$restUC." $userachDffOpt $vsearchSpcfcOpt";#-threads  $BlastCores";
		if (system($cmd) != 0){printL ("Failed: $cmd\n",1); }
		system("cat $restUC >> ".$UCguide[0].".RESTREF");unlink $restUC;
	}
}
#if (system("cat ".join(" ",@allUCs)." >> ".$UCguide[0]) != 0){printL "Merge of UC subfiles failed.\n",1;};
#foreach (@allUCs){unlink;}
...
sub derepBash($){
	...
	sed -e \'s/\_/\n/2\' > ' . "$t/derep.fa"; #amplicons_linearized_dereplicated.fasta
	die $derepCmd."\n";
...
sub swarmClust($){
	...
	my $derepl = "$t/derep.fas";#,$totSeqs,$arL)
	if (!-f $derepl || -z $derepl){printL "The sdm dereplicated output file was either empty or not existant, aborting lotus.\n$derepl\n",1;}
